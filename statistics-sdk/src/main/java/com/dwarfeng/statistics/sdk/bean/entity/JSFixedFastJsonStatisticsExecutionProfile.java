...
/**
 * JSFixed FastJson 统计执行简报。
 *
 * @author DwArFeng
 * @since 1.0.0
 */
public class JSFixedFastJsonStatisticsExecutionProfile implements Bean {

    private static final long serialVersionUID = 4219090948903462738L;

    public static JSFixedFastJsonStatisticsExecutionProfile of(StatisticsExecutionProfile statisticsExecutionProfile) {
        if (Objects.isNull(statisticsExecutionProfile)) {
            return null;
        } else {
            return new JSFixedFastJsonStatisticsExecutionProfile(
                    JSFixedFastJsonLongIdKey.of(statisticsExecutionProfile.getKey()),
    ...
    public JSFixedFastJsonStatisticsExecutionProfile() {
    }

    public JSFixedFastJsonStatisticsExecutionProfile(
            JSFixedFastJsonLongIdKey key, int executingCount, int succeededCount, int failedCount,
            int consecutiveFailedCount, Date lastExecutedDate, Date lastSucceededDate, Date lastFailedDate
    ) {
        this.key = key;
    ...
    public String toString() {
        return "JSFixedFastJsonStatisticsExecutionProfile{" +
                "key=" + key +

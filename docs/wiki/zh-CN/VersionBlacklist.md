...
- 是否直接或间接使用了被提及的模块/类（例如：`com.dwarfeng.statistics.node.launcher.Launcher`）。
- 业务中是否存在与“原因”描述相符的顺序敏感或易触发场景（例如：程序启动时）。
...
| 编号                                           | 大版本   | 起始版本    | 结束版本    | 原因                               |
|----------------------------------------------|-------|---------|---------|----------------------------------|
| [BLACKLIST-20260125.1](#BLACKLIST-202601251) | 1.3.x | 1.3.0.a | 1.3.0.a | 程序启动时不能正确解析 `opt/opt-*.xml` 配置文件 |

## 详细原因

### BLACKLIST-20260125.1

原因：程序启动器存在 bug，使用 `classpath:` 前缀加载 `opt/opt-*.xml` 配置文件时无法正确解析，导致程序无法正常启动。
...
- 典型症状：
  - 程序启动失败。
  - Spring 上下文初始化失败，无法找到在 `opt/opt-*.xml` 中配置的 bean。
- 影响范围：
  - 所有使用 `opt/opt-*.xml` 配置文件进行功能配置的场景。
  - 程序启动流程。

迁移建议：升级至 1.3.1.a 及以上版本。

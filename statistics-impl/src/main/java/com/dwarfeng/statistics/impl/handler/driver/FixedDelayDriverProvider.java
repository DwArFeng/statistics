...
import com.dwarfeng.statistics.sdk.handler.driver.AbstractDriver;
...
public class FixedDelayDriverProvider implements DriverProvider {
    ...
    public static class FixedDelayDriver extends AbstractDriver {
        ...
        public void register(DriverInfo driverInfo) throws DriverException {
            ...
            try {
                ...
                FixedDelayProcessor fixedDelayProcessor = new FixedDelayProcessor(context, statisticsSettingKey);
    ...
    private static class FixedDelayProcessor implements Runnable {
        ...
        private FixedDelayProcessor(Driver.Context context, LongIdKey statisticsSettingKey) {
        ...
        public void run() {
            ...
            try {
                ...
                LOGGER.debug("计划时间已到达, fixed delay 驱动器驱动统计设置 {} 执行统计动作...", statisticsSettingKey);
                context.execute(statisticsSettingKey);
            } catch (Exception e) {
                LOGGER.warn("驱动设置 {} 执行统计动作时出现异常, 放弃本次记录", statisticsSettingKey, e);
            } finally {
                lock.unlock();
            }
        }
